...
import ReactTweet from 'react-tweet-embed'
...
This means that you can now leverage the power of React components to create AMP pages.

AMP support is defined on a per-page basis, allowing incremental adoption of AMP.

There are two ways to enable AMP in Next.js: a Hybrid AMP page or an AMP-only page.

## Hybrid AMP Pages

Hybrid AMP pages allow you to have an accompanying AMP version of a traditional page.

This allows for search engines to show the AMP version of the page in the mobile search results while maintaining the existing version of the page.

This allows the use of AMP into your application while still making use of Next.js features like client-side routing for the main user experience.

To opt-in to a hybrid AMP page use the `withAmp` function while providing the `hybrid: true` option.

`withAmp` can be used on pages in the pages directory to enable hybrid AMP:
...
AMP-only pages are served to the website's primary traffic as well as search engine traffic.

By using AMP-only pages, we bring the fast experience of AMP to the main website, including desktop.

To implement to AMP-only pages, you wrap pages with the `withAmp` function:
...
If you don't have a need for the Next.js runtime, using AMP-only pages could help speed up development. AMP-only requires you to only use AMP compatible components to build your pages.

AMP-only pages are currently in production on [nextjs.org](http://nextjs.org)/docs and [nextjs.org/blog](http://nextjs.org/blog).

### Distinguishing AMP Rendering in Components

Any React component in your project can leverage `useAmp` to distinguish between AMP and non-AMP rendering mode.

This allows you to implement an `<Image>` component that shares logic between `<img>` and `<amp-img>`:
