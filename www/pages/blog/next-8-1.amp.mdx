...
- [AMP in Next.js](#amp-in-next.js)
- [Hybrid AMP pages](#hybrid-amp-pages)
- [AMP-only pages](#amp-only-pages)
...
AMP-only pages are served to the website's primary traffic as well as search engine traffic. By using AMP-only pages, we bring the fast experience of AMP to the main website, including desktop.
...
If you don't have a need for the Next.js runtime, using AMP-only pages could help speed up development. AMP-Only requires you to only use AMP compatible components to build your pages. AMP-Only pages are currently in production on [nextjs.org/docs](http://nextjs.org/docs) and [nextjs.org/blog](http://nextjs.org/blog).
